pipeline {
	...
	stage('Build'){
	    steps {
		    sh "sudo docker build /home/ubuntu/jenkins/workspace/${JOB_NAME} -t skycolourlife/devopsdemo"
	   }
	}
	stage('Docker Push'){
		steps {
		    sh "sudo docker login --username skycolourlife --password ${dockerpass}"
                    sh "sudo docker push skycolourlife/devopsdemo:latest"
	        }
	}
